  ...
  function(t_Fix = NULL,
    ...
    if (!is.null(precision.dp)) {
      ...
      if (decimalplaces(obs_d13C) == 0) {
        warning(
          'Precision assumed to be 0 d.p. due to provided value of "obs_d13C" (e.g., -24.00 is read as -24 rather than being at 2 d.p.).  Provide precision using "precision.dp" instead.',
          call. = FALSE,
          immediate. = TRUE
        )
      }
    ...
    if (!is.null(t_Fix)) {
      ...
      if (is.infinite(t_EStor_mx) | t_EStor_mx < 0) {
        t_EStor_mx <- 0
      }
...
Hindcast_d13C <- function(obs_d13C,
  ...
  if (is.null(specimenID)) {
    specimenID <- 1:length(obs_d13C)
  }
  
  if (!missing(model) &
      length(model) > 1)
    stop("Only one 'model' allowed at a time.")
  
  model <- match.arg(model, choices = c('Biphasic', 'Monophasic'))
  
  if (createTimeSeries & length(obs_d13C) > 1) {
    stop(paste(
      'A time series can only be created for single sample but you specified ',
      length(obs_d13C)
    ))
  }
  ...
  if (all(c(is.null(obs_d13C_SD), is.null(obs_d13C_SE)))) {
    stop(
      "Provide either Standard Deviation or Standard Error (and sample size) of lab standard or control samples in order apply it to focal sample."
    )
  }
  ...
  if (length(obs_d13C_SD) == 1 & length(obs_d13C) > 1) {
    warning(
      'Using same Standard Deviation for all samples.  If not desired, specify SD or Standard Error (and sample size) of control samples for each sample.',
      call. = FALSE,
      immediate. = TRUE
    )
  ...
  if (any(t_Pres != t_Fix + t_EStor)) {
    warning(
      "The durations of fixation and ethanol storage don't all add up to the provided total preservation time.",
      call. = FALSE,
      immediate. = TRUE
    )
  }
  ...
  if (length(obs_d13C) != length(t_Pres) |
    ...
    warning(
      "Using same time periods of total preservation, fixation and storage for all samples. If not desired, provide them for each sample.",
      call. = FALSE,
      immediate. = TRUE
    )
  ...
  if (is.null(precision.dp)) {
    warning(
      "No precison for obs_d13C provided, so using decimal places of obs_d13C.",
      call. = FALSE,
      immediate. = TRUE
    )
  }
  ...
  if (model == 'Biphasic') {
    hmod <-
      function(t_Fix,
        ...
        if (!naive &
            t_Fix < mx_times['t_Fix_mx'] & t_EStor >= mx_times['t_EStor_mx']) {
          n_t_Fix <- t_Fix
  ...
  if (!is.null(fit)) {
    ...
    if (model == 'Monophasic' & length(fixef) != 2) {
      stop(
        "The number of parameters estimated in the provided model fit
  ...
  if (is.null(fit)) {
    # hard-coded estimates from Taylor et al. data
    if (model == 'Monophasic') {
  ...
  pred.point <-
    pred.mean <- pred.median <- t_Pres_mx <- rep(NA, length(obs_d13C))
  ...
  for (i in 1:length(obs_d13C)) {
    # this loop could be converted to apply-style
    pars.resamp <-
      cbind(obs_d13C = rnorm(reSamps, obs_d13C[i], obs_d13C_SD[i]),
            pars.resamp.e)
    ...
    if (model == 'Monophasic') {
      ...
      preds.adj <-
        apply(pars.resamp, 1, function(x) {
          x['obs_d13C'] -
            hmod(
              t_Pres = t_Pres_obs[i],
              obs_d13C = x['obs_d13C'],
