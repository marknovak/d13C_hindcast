...
dfCI <-
  data.frame(t(apply(preds, 1, quantile, c(0.025, 0.5, 0.975))))
dfM <- apply(preds, 1, mean)

#Combine output in data frame
fit1Cr.Predictions <- data.frame(
  ndat,
  Pred = mod1(ndat$Pres,
              fixef['I'],
              fixef['A'],
...
ylims_u <-
  c(0, max(c(
    fit1Cr.Predictions$u95 - fit1Cr.Predictions$l95
  )))
...
ylims_f <- range(
  c(
    fit1Cr.Predictions$u95,
    fit1Cr.Predictions$l95,
    fit2Cr.Predictions$u95,
    fit2Cr.Predictions$l95
  )
)
