...
ldat.sd <-
  as.numeric(names(sort(table(ldat$d13C_sd), decreasing = TRUE)[1]))
...
# Predicted time (in years) to reach asymptote given precision:
ts.preds$t_Pres_mx[1] / 365
...
fdat.preds <- merge(fdat[, c('SpecimenID',
                             'd15N',
                             'd13C',
                             'Fix',
                             'EStor',
...
ylims <-
  range(c(ldat.mean, ts.preds$pred.CI.2.5, ts.preds$pred.CI.97.5))
plot(
  ts.preds$t_Pres,
  ts.preds$pred.mean,
...
polygon(
  c(ts.preds$t_Pres, rev(ts.preds$t_Pres)) ,
...
trans <-
  ts.preds$t_Pres <= ts.preds$t_Pres_mx[1] # to indicate asymptote precision transition
# trans<-ts.preds$t_Fix<max(ts.preds$t_Fix) # to indicate fixation transition
polygon(
  c(ts.preds$t_Pres[trans], rev(ts.preds$t_Pres[trans])) ,
...
lines(ts.preds$t_Pres, ts.preds$pred.point,
      lwd = 2)
points(ts.preds$t_Pres[1],
       ts.preds$pred.point[1],
       pch = 19,
       lwd = 2)
...
arrows(
  fdat.preds$d13C,
  fdat.preds$pred.CI.2.5,
  fdat.preds$d13C,
  fdat.preds$pred.CI.97.5,
  length = 0,
  code = 3,
  angle = 90
)
...
title(xlab = expression(paste('Actual initial ', delta ^ 13, 'C')), line =
        1.5)
legend(
  'bottomright',
  legend = 'B',
