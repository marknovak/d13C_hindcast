...
odat<-read.csv('../Data/Taylor_FormalinFixation.csv',header=T,stringsAsFactors=FALSE)
...
odat$d13C <- apply(cbind(odat$d13C_Run1, odat$d13C_Run2),1,mean, na.rm=TRUE)
odat$d13C_sd <- apply(cbind(odat$d13C_sd_Run1, odat$d13C_sd_Run2),1,mean, na.rm=TRUE)
odat$d13C_sd_n <- apply(cbind(odat$d13C_sd_n_Run1, odat$d13C_sd_nRun2),1,mean, na.rm=TRUE)
...
dat <- odat[,c('Collector_Number','Source',
               'Fixation_days','Storage_days','Preservation_days',
               'd13C','d13C_sd','d13C_sd_n',
               'd15N','d15N_sd','d15N_sd_n')]
               
# Abbreviate predictors
colnames(dat)<-sub('Collector_Number','SpecimenID',colnames(dat))
