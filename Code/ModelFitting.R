...
starts <-
  list(nlpars = apply(coef(fit1C), 2, mean), theta = vcov(fit1C))
...
fit2N <- nlme(
  d15N ~ I + A * (1 - exp(l1 * Fix) * exp(l2 * EStor)),
  fixed = I + A + l1 + l2 ~ 1,
  random = I ~ 1 | SpecimenID,
  start = starts,
  data = dat,
...
fit2C <- nlme(
  d13C ~ I + A * (1 - exp(l1 * Fix) * exp(l2 * EStor)),
  fixed = I + A + l1 + l2 ~ 1,
  random = I ~ 1 | SpecimenID,
  start = starts,
  data = dat,
